...
class virchow2(nn.Module):
    def __init__(self):
        ...
        self.virchow2 = timm.create_model("hf-hub:paige-ai/Virchow2", pretrained=True, mlp_layer=SwiGLUPacked, act_layer=torch.nn.SiLU)
    
    def forward(self, x):
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.float16):
            output = self.virchow2(x)# 1, 261, 1280
