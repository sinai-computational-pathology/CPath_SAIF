import datasets_mccv as datasets
import modules
import os
import argparse
...
parser.add_argument('--nepochs', type=int, default=50, help='number of epochs (default: 50)')
...
parser.add_argument('--master_path', type=str, default='./master_metadata_mccv.csv', help='Path to the master metadata file')
parser.add_argument('--roi_metadata', type=str, default='./roi_metadata.csv', help='Path to the ROI metadata file')
...
parser.add_argument('--task_label', type=str, default='self_reported', help='Type of label to use (e.g., "genetic_ancestry", "self_reported")')
...
def main():
    ...
    # Set datasets
    train_dset, val_dset, weights, class_names = datasets.get_datasets_trident(
    ...
    # Get model
    num_classes = int(weights.numel()) if hasattr(weights, 'numel') else len(weights)
    print(f"Using num_classes={num_classes} based on dataset weights")
    if args.aggregator == 'GMA':
        model = modules.GMA(dropout=True, n_classes=num_classes, n_tasks=1, ndim=args.ndim)
    elif args.aggregator == 'GMA_multiple':
        model = modules.GMA_multiple(dropout=True, n_classes=num_classes, n_tasks=num_classes, ndim=args.ndim)
    else:
    ...
    if os.path.exists(checkpoint_path):
        ...
        for epoch in range(args.nepochs+1):
            ...
            targets = val_loader.dataset.df.target
            ...
            if len(np.unique(targets)) == 2:
                ...
                if probs.ndim == 2 and probs.shape[1] > 1:
                    ...
                    auc_ovr = roc_auc_score(targets, probs[:, 1])
                else:
                    auc_ovr = roc_auc_score(targets, probs)
                auc_ovo = auc_ovr  # For binary, ovr == ovo
            else:
                # Multiclass
                auc_ovr = roc_auc_score(targets, probs, multi_class='ovr')
                auc_ovo = roc_auc_score(targets, probs, multi_class='ovo')
    ...
    _, val_dset, _, class_names = datasets.get_datasets_trident(
        encoder=args.encoder, 
        task='attention', 
        class_only=args.class_only, 
        task_label=args.task_label, 
    ...
    attention_scores_dict, slide_output = inference(val_loader, model, args.task_label, device, class_names=class_names)
...
def inference(loader, model, task_label, device, class_names=None):
    # Set model in evaluation mode
    model.eval()

    attention_scores_dict = {}  # To store attention scores per slide with slide_id as the key
    all_probs = []              # To store probabilities for each slide
    all_slide_info = []         # To store slide-level info (e.g., labels)

    # Determine columns: prefer provided class_names, otherwise leave None to let pandas infer
    columns = class_names

    with torch.no_grad():
        for i, batch in enumerate(loader):
            # Support DataLoader returning either (input, ...) or just input
            ...
            # Safely access row information from the dataset dataframe
            row = loader.dataset.df.iloc[i]
            target = row['target']
            slide = row['slide']
            barcode = row['barcode'] if 'barcode' in loader.dataset.df.columns else None

            # Get attention scores (may be tensor or numpy)
            attention_scores = model(input, attention_only=True)
            ...
            # Convert attention scores to a consistent numpy array shape: [num_tiles, n_tasks]
            try:
                att = attention_scores
                if isinstance(att, torch.Tensor):
                    att = att.detach().cpu().numpy()
                # If shape is (n_tasks, num_tiles), transpose to (num_tiles, n_tasks)
                if hasattr(att, 'ndim') and att.ndim == 2 and att.shape[0] <= att.shape[1]:
                    att = att.T
                attention_scores_dict[slide] = att
            except Exception:
                # Fallback: store raw attention scores
                attention_scores_dict[slide] = attention_scores
    ...
    slide_probs_df = pd.DataFrame(all_probs, columns=columns)
...
def test(run, loader, model):
    ...
    model.eval()
    # Build probability list dynamically to handle variable number of classes
    probs_list = []
    ...
    with torch.no_grad():
        for i, (input, _) in enumerate(loader):
            ...
            ## Append per-slide probabilities (assume batch size 1)
            probs_list.append(output.detach().cpu().squeeze(0).numpy())
    # Stack to (N, C)
